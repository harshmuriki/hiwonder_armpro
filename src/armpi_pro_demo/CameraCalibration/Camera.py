...
class CameraROSNode:
    def __init__(self):
        ...
        self.img_counter = 0
        rospy.loginfo("Camera node started, publishing to /camera/image_raw and waiting for capture commands on /camera/capture.")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.camera.frame is not None:
                try:
                    image_message = self.bridge.cv2_to_imgmsg(self.camera.frame, encoding="bgr8")
